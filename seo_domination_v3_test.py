...
def run():
    ...
    print(f"Discovered {len(target_dirs)} potential target directories.")
    
    test_subset = random.sample(target_dirs, min(5, len(target_dirs)))
    
    modified_count = 0
    for directory in test_subset:
        index_file = os.path.join(directory, "index.html")
        if os.path.exists(index_file):
            print(f"Modifying {index_file}...")
            if process_file(index_file):
                modified_count += 1
                
    print(f"DRY RUN COMPLETED. Modified {modified_count} static pages. Verify the HTML structure.")
