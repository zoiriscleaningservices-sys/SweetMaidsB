$locations = @(
    "Tampa", "Davis Islands", "Harbour Island", "Palma Ceia", "Hyde Park", 
    "Culbreath Isles", "Sunset Park", "Bayshore Beautiful", "Beach Park", 
    "New Suburb Beautiful", "South Tampa", "Parkland Estates", "Virginia Park", 
    "Avila", "Odessa", "Cheval", "Westchase", "Cory Lake Isles", "Arbor Greene", 
    "Grand Hampton", "New Tampa", "Apollo Beach", "Fishhawk", "Lithia", "Lutz", 
    "Temple Terrace", "Carrollwood", "Keystone", "Old Memorial Estates", 
    "Northbridge at Lake Pretty", "Palma Ceia West", "Snell Isle", "Sunset Pointe", 
    "Bayshore Gardens"
)
...
foreach ($loc in $locations) {
    ...
    $tampaLinksHeader += "<a href=""/$locSlug/"" class=""block px-3 py-2 rounded-lg hover:bg-gradient-to-r hover:from-pink-50 hover:to-pink-50 text-gray-700 hover:text-pink-300 font-medium text-xs transition"">$loc</a>`n"
    $tampaLinksMobile += "<a href=""/$locSlug/"" class=""mobile-link py-2 text-sm text-gray-600 hover:text-pink-400"">$loc</a>`n"
    $tampaLinksFooter += "<a href=""/$locSlug/"" class=""hover:text-pink-400 transition-colors"">$loc</a>`n"
}
...
<div class="text-xs font-bold text-pink-300 uppercase tracking-wider px-4 py-2 mb-2">Tampa Service Areas</div>
...
<span class="w-1 h-6 bg-pink-300 rounded-full"></span>Tampa Service Areas
</h4>
...
"@)
    ...
    $content = $content -replace '(?s)<meta name="description".*?>', "<meta name=""description"" content=""Looking for the best cleaning service in $cleanLoc, FL? Sweet Maid offers top-rated luxury house cleaning, deep cleaning, and move-out services in Tampa. Licensed, insured, and 100% satisfaction guaranteed. Book your sparkle today!"" />"
    ...
    $content = $content -replace '<meta property="og:description" content=".*?"', "<meta property=""og:description"" content=""Looking for the best cleaning service in $cleanLoc, FL? Sweet Maid offers top-rated luxury cleaning in Tampa's finest neighborhoods."""
    ...
    $content = $content -replace '"latitude": "27.4989"', '"latitude": "27.9506"'
    $content = $content -replace '"longitude": "-82.5748"', '"longitude": "-82.4572"'
...
"@)
    $content = [regex]::Replace($content, '(?s)<p class="text-xl text-gray-600 mb-10 max-w-lg leading-relaxed">.*?</p>', @"
          <p class="text-xl text-gray-600 mb-10 max-w-lg leading-relaxed">
            Tampa's most trusted luxury cleaning service arrives in $cleanLoc. We bring the sparkle back to your $cleanLoc home so you can focus on the vibrant Tampa Bay lifestyle.
          </p>
"@)
    ...
    $content = $content -replace "Bradenton's", "$cleanLoc's"
    $content = $content -replace "across Bradenton and Southwest Florida", "across $cleanLoc and the Tampa area"
    ...
    # MAP Optimization
    $mapPb = "!1m18!1m12!1m3!1d3592.518!2d-82.4572!3d27.9506!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x88d9b0a5015b3c53%3A0xc074744040974837!2s$($loc -replace ' ', '+')%2C+FL%2C+USA!5e0!3m2!1sen!2sus!4v1739561081827!5m2!1sen!2sus"
...
"@)
        ...
        $baseContent = $baseContent -replace "Bradenton's", "$cleanLoc's"
        $baseContent = $baseContent -replace "across Bradenton and Southwest Florida", "across $cleanLoc and the Tampa area"
        ...
        $baseContent = $baseContent -replace '"latitude": "27.4989"', '"latitude": "27.9506"'
        $baseContent = $baseContent -replace '"longitude": "-82.5748"', '"longitude": "-82.4572"'
...
"@)
            ...
            $svcContent = $svcContent -replace '(?s)<meta name="description".*?>', "<meta name=""description"" content=""Need the best $svcName in $cleanLoc, FL? Sweet Maid offers expert, luxury cleaning services in the Tampa area. Licensed & Insured. 100% Guaranteed. Get a Quote!"" />"
            ...
            $svcContent = $svcContent -replace '<meta property="og:description" content=".*?"', "<meta property=""og:description"" content=""Need professional $svcName in $cleanLoc? Sweet Maid offers top-rated luxury services across Tampa's finest enclaves."""
            ...
            $svcContent = $svcContent -replace '"latitude": "27.4989"', '"latitude": "27.9506"'
            $svcContent = $svcContent -replace '"longitude": "-82.5748"', '"longitude": "-82.4572"'
...
"@)
            ...
            $svcContent = $svcContent -replace "Bradenton's", "$cleanLoc's"
            $svcContent = $svcContent -replace "across Bradenton and Southwest Florida", "across $cleanLoc and the Tampa area"
...
Write-Host "Tampa Expansion Complete!"
