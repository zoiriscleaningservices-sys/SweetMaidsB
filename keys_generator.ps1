$locations = @(
    "Key Largo", "Tavernier", "Islamorada", "Plantation Key", "Matecumbe Key", 
    "Marathon", "Duck Key", "Key Colony Beach", "Grassy Key", "Conch Key", 
    "Long Key", "Big Pine Key", "Little Torch Key", "Ramrod Key", "Summerland Key", 
    "Cudjoe Key", "Sugarloaf Key", "Big Coppitt Key", "Stock Island", "Rock Harbor", 
    "Key West", "The Florida Keys", "Ocean Reef", "Port Largo", "Largo Sound Village", 
    "Ocean Shores", "Anglers Park", "Upper Matecumbe Key", "Lower Matecumbe Key", 
    "Windley Key", "Pen Key Club", "Hawks Cay", "Indigo Reef", "Sunset Pointe", 
    "Little Palm Island", "Shark Key", "Sunset Key", "Truman Annex", "Casa Marina", 
    "No Name Key", "Saddlebunch Keys", "Geiger Key", "Boca Chica Key", 
    "Sugarloaf Shores", "Bay Point Keys"
)
...
foreach ($loc in $locations) {
    ...
    $keysLinksHeader += "<a href=""/$locSlug/"" class=""block px-3 py-2 rounded-lg hover:bg-gradient-to-r hover:from-pink-50 hover:to-pink-50 text-gray-700 hover:text-pink-300 font-medium text-xs transition"">$loc</a>`n"
    $keysLinksMobile += "<a href=""/$locSlug/"" class=""mobile-link py-2 text-sm text-gray-600 hover:text-pink-400"">$loc</a>`n"
    $keysLinksFooter += "<a href=""/$locSlug/"" class=""hover:text-pink-400 transition-colors"">$loc</a>`n"
}
...
<div class="text-xs font-bold text-pink-300 uppercase tracking-wider px-4 py-2 mb-2">Florida Keys Service Areas</div>
...
<span class="w-1 h-6 bg-pink-300 rounded-full"></span>Florida Keys Service Areas
</h4>
...
"@)
    ...
    $content = $content -replace '(?s)<meta name="description".*?>', "<meta name=""description"" content=""Looking for the best cleaning service in $cleanLoc, FL? Sweet Maid offers top-rated house cleaning, deep cleaning, and move-out services in the Florida Keys. Licensed, insured, and 100% satisfaction guaranteed. Book your sparkle today!"" />"
    ...
    $content = $content -replace '<meta property="og:description" content=".*?"', "<meta property=""og:description"" content=""Looking for the best cleaning service in $cleanLoc, FL? Sweet Maid offers top-rated house cleaning, deep cleaning, and move-out services in the Florida Keys."""
    ...
    $content = $content -replace '"latitude": "27.4989"', '"latitude": "24.5551"'
    $content = $content -replace '"longitude": "-82.5748"', '"longitude": "-81.7800"'
...
"@)
    $content = [regex]::Replace($content, '(?s)<p class="text-xl text-gray-600 mb-10 max-w-lg leading-relaxed">.*?</p>', @"
          <p class="text-xl text-gray-600 mb-10 max-w-lg leading-relaxed">
            Most trusted cleaning service in $cleanLoc. We bring the sparkle back to your $cleanLoc home so you can focus on what matters most in the Florida Keys.
          </p>
"@)
    ...
    $content = $content -replace "Bradenton's", "$cleanLoc's"
    $content = $content -replace "across Bradenton and Southwest Florida", "across $cleanLoc and the Florida Keys"
...
"@)
        ...
        $baseContent = $baseContent -replace "Bradenton's", "$cleanLoc's"
        $baseContent = $baseContent -replace "across Bradenton and Southwest Florida", "across $cleanLoc and the Florida Keys area"
        ...
        $baseContent = $baseContent -replace '"latitude": "27.4989"', '"latitude": "24.5551"'
        $baseContent = $baseContent -replace '"longitude": "-82.5748"', '"longitude": "-81.7800"'
...
"@)
            ...
            $svcContent = $svcContent -replace '(?s)<meta name="description".*?>', "<meta name=""description"" content=""Need the best $svcName in $cleanLoc, FL? Sweet Maid offers expert, affordable, and top-rated cleaning services in the Florida Keys. Licensed & Insured. 100% Guaranteed. Get a Quote!"" />"
            ...
            $svcContent = $svcContent -replace '<meta property="og:description" content=".*?"', "<meta property=""og:description"" content=""Need professional $svcName in $cleanLoc? Sweet Maid offers top-rated services across the Florida Keys."""
            ...
            $svcContent = $svcContent -replace '"latitude": "27.4989"', '"latitude": "24.5551"'
            $svcContent = $svcContent -replace '"longitude": "-82.5748"', '"longitude": "-81.7800"'
...
"@)
            ...
            $svcContent = $svcContent -replace "Bradenton's", "$cleanLoc's"
            $svcContent = $svcContent -replace "across Bradenton and Southwest Florida", "across $cleanLoc and the Florida Keys"
...
Write-Host "Florida Keys Expansion Complete!"
