...
$locationsHtml = @"
  <!-- ================================================
     AREAS & MAP
=============================================== -->
  <section overflow-hidden id="areas" class="py-24 relative overflow-hidden bg-white">
...
if ($startIndex -eq -1) {
    Write-Host "Could not find AREAS & MAP anchor."
    exit 1
}

# Go backwards to find the start of the comment block
$commentStart = $content.LastIndexOf("<!--", $startIndex)
if ($commentStart -eq -1) {
    Write-Host "Could not find start comment for areas section."
    exit 1
}
...
if ($endIndex -eq -1) {
    Write-Host "Could not find closing section"
    exit 1
}

$endIndex += $endStr.Length

$before = $content.Substring(0, $commentStart)
$after = $content.Substring($endIndex)
$newContent = $before + $locationsHtml + $after

[System.IO.File]::WriteAllText($filePath, $newContent, (New-Object System.Text.UTF8Encoding $false))

Write-Host "Successfully replaced the Areas section."
