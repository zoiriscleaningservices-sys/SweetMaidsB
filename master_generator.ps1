...
foreach ($name in $areaNames) {
    ...
    $title = "Best Cleaning Services in $cleanName FL | $cleanName Cleaning Services"
    $desc = "Looking for the best cleaning services in $cleanName FL? Sweet Maid provides top-rated $cleanName cleaning services. We are the premier choice for cleaning services $cleanName fl."
    $keywords = "cleaning services in $cleanName fl, $cleanName cleaning services, cleaning services $cleanName fl, best house cleaning $cleanName, top rated maid service $cleanName"
    ...
    $localHeader = $localHeader -replace '<a href="#" onclick="window.scrollTo\(\{top: 0, behavior: ''smooth''\}\); return false;"(.*?>Home</a>)', "<a href=`"/$locSlug/`"`$1"
    $localMenu = $localMenu -replace '<a href="#" onclick="window.scrollTo\(\{top: 0, behavior: ''smooth''\}\); return false;"(.*?>\s*(?:<span.*?>)?Home(?:</span>)?\s*<i.*?>.*?)</a>', "<a href=`"/$locSlug/`"`$1</a>"
...
"@)
    ...
    $localMain = [regex]::Replace($localMain, '(?s)<p class="text-xl text-gray-600 mb-10 max-w-lg leading-relaxed">.*?</p>', @"
          <p class="text-xl text-gray-600 mb-10 max-w-lg leading-relaxed">
            Leading <strong>cleaning services in $cleanName fl</strong>. We bring the sparkle back to your home. Looking for the best <strong>$cleanName cleaning services</strong>? You found us! We provide elite <strong>cleaning services $cleanName fl</strong>.
          </p>
"@)
